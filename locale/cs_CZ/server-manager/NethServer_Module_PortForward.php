<?php 

/* NethServer_Module_PortForward translation, language: en */

$L['Allow_label'] = 'Umožnit pouze z';
$L['check-rules_Header'] = 'Zkontrolovat pravidla brány firewall';
$L['CheckRules_label'] = 'Zkontrolovat bránu firewall';
$L['create_header_label'] = 'Vytvořit předávání portu';
$L['Description_label'] = 'Popis';
$L['disable_label'] = 'Vypnout';
$L['DstHost_label'] = 'Cílový stroj';
$L['DstRaw_label'] = 'Cílový stroj';
$L['Dst_label'] = 'Cílový port';
$L['enable_label'] = 'Zapnout';
$L['Key_label'] = 'Identifikátor';
$L['PortForward_Description'] = 'Spravovat přeposílání portu';
$L['PortForward_Tags'] = 'port portforward forward hairpin';
$L['PortForward_Title'] = 'Předávání portu';
$L['Proceed with port forward disable?'] = 'Opravdu nepřeposílat porty?';
$L['Proceed with port forward enable?'] = 'Zapnout předávání portů?';
$L['Proto_label'] = 'Protokol';
$L['Src_label'] = 'Port ze kterého pochází';
$L['tcp_label'] = 'TCP';
$L['update_header_label'] = 'Upravit předávání portů';
$L['udp_label'] = ' ';
$L['tcp,udp_label'] = 'TCP, UDP';
$L['OriDst_label'] = "IP adresa WAN (červené rozhraní)";
$L['any_label'] = "Libovolné";
$L['PickDestination_label'] = 'Zvolte cíl…';
$L['HairpinNat_label'] = 'Hairpin NAT';
$L['valid_platform,firewall-object-exists,uniq-fwobject,255'] = 'Je třeba vyplnit kolonku cílový stroj.';
$L['duplicate_pfw'] = 'Už existuje přeposílán portu se stejnou WAN IP adresou a portem původu.';
$L['Log_label'] = 'Pokud je toto pravidlo uplatněno, zapsat do záznamu událostí';
