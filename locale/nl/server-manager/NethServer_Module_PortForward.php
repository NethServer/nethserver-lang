<?php 

/* NethServer_Module_PortForward translation, language: en */

$L['Allow_label'] = 'Alleen toestaan van';
$L['check-rules_Header'] = 'Controleer firewall regels';
$L['CheckRules_label'] = 'Controleer firewall';
$L['create_header_label'] = 'Creëer port forward';
$L['Description_label'] = 'Beschrijving';
$L['disable_label'] = 'Uitschakelen';
$L['DstHost_label'] = 'Destination host';
$L['DstRaw_label'] = 'Destination host';
$L['Dst_label'] = 'Destination poort';
$L['enable_label'] = 'Inschakelen';
$L['Key_label'] = 'Id';
$L['PortForward_Description'] = 'Beheer port forwarding';
$L['PortForward_Tags'] = 'port portforward forward hairpin';
$L['PortForward_Title'] = 'Port Forwarding';
$L['Proceed with port forward disable?'] = 'Doorgaan met uitschakelen van port forward?';
$L['Proceed with port forward enable?'] = 'Doorgaan met inschakelen van port forward?';
$L['Proto_label'] = 'Protocol';
$L['Src_label'] = 'Origin port';
$L['tcp_label'] = 'TCP';
$L['update_header_label'] = 'Wijzig port forward';
$L['udp_label'] = 'UDP';
$L['tcp,udp_label'] = 'TCP,UDP';
$L['OriDst_label'] = "WAN IP (rood interface)";
$L['any_label'] = "Iedere";
$L['PickDestination_label'] = 'Kies bestemming';
$L['HairpinNat_label'] = 'Hairpin NAT';
$L['valid_platform,firewall-object-exists,uniq-fwobject,255'] = 'Het veld \'destination host\' mag niet leeg zijn';
$L['duplicate_pfw'] = 'Er bestaat al een port forward met hetzelfde WAN IP en origin port';
$L['Log_label'] = 'Schrijf naar log als deze regel overeenkomt';
